...
class App(tk.Tk):
    ...
    def _make_ui(self):
        ...
        row = 0
        for lab, val in [(self.lab_tor, self.val_tor),
        ...
        for i in range(4): btnf.grid_columnconfigure(i, weight=1)

        ttk.Button(btnf, text="Re-check", command=self._refresh_async).grid(row=0, column=0, sticky="we", padx=4)
        ttk.Button(btnf, text="Open Tor Check", command=lambda: webbrowser.open("https://check.torproject.org/")).grid(row=0, column=1, sticky="we", padx=4)
        ttk.Button(btnf, text="HTTP test (HTTPS-Only)", command=lambda: webbrowser.open("http://http.badssl.com/")).grid(row=0, column=2, sticky="we", padx=4)
        ttk.Button(btnf, text="WebRTC test", command=lambda: webbrowser.open("https://browserleaks.com/webrtc")).grid(row=0, column=3, sticky="we", padx=4)

        ttk.Separator(self).grid(row=row+1, column=0, columnspan=2, sticky="we", padx=10, pady=4)
        ttk.Label(self, text="Tip: This app never reads your pages. It only runs test requests via Tor.").grid(row=row+2, column=0, columnspan=2, sticky="w", padx=10, pady=4)
